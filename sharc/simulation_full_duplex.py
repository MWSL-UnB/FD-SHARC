...
class SimulationFullDuplex(Simulation):
    ...
    def calculate_sinr_ext(self):
        ...
        self.ue.sinr_ext[ue] = self.ue.rx_power[ue] \
                               - (10 * np.log10(
            np.power(10, 0.1 * self.ue.total_interference[ue]) + np.power(10, 0.1 * self.ue.ext_interference[ue])))
        ...
        for bs in bs_active:
            ...
            self.bs.sinr_ext[bs] = self.bs.rx_power[bs] \
                                   - (10 * np.log10(
                np.power(10, 0.1 * self.bs.total_interference[bs]) + np.power(10, 0.1 * self.bs.ext_interference[bs])))
    ...
    def collect_results(self, write_to_file: bool, snapshot_number: int):
        if not self.parameters.imt.interfered_with:
            ...
            self.results.system_ul_inr_scaled.extend(
                (self.system_ul_inr + 10 * math.log10(self.param_system.inr_scaling)).tolist())
            self.results.system_dl_inr_scaled.extend(
                (self.system_dl_inr + 10 * math.log10(self.param_system.inr_scaling)).tolist())
