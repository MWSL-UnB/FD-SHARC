...
class StationFactory(object):
    ...
    def generate_imt_base_stations(param: ParametersImt,
        ...
        imt_base_stations.interference_from_bs = dict([(bs, -500 * np.ones(param.ue_k)) for bs in range(num_bs)])
        imt_base_stations.interference_from_ue = dict([(bs, -500 * np.ones(param.ue_k)) for bs in range(num_bs)])
    ...
    def generate_system(parameters: Parameters, topology: Topology, random_number_gen: np.random.RandomState ):
        ...
        elif parameters.general.system == "NONE":
            return None
        else:
